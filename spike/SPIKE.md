...
5. **Wire a minimal Jaspr `ContentApp`** in `spike/web/` (or
   `example/spike/`, your call — document where in `DEBRIEF.md`). Configure
   it with `parsers: [PandocParser()]`, `extensions:
   [TableOfContentsExtension()]`, a `DocsLayout`. Point the route loader
   at `spike/article.pandoc.json`.
6. **Run it.** `dart run jaspr serve` (or whatever the current command
   is). Open the page. Read what works and what looks wrong.
...
- The article renders in a browser at the standard Jaspr dev port.
  Headers, paragraphs, bullet and numbered lists, emphasis, strong,
  links, and the image reference all appear. Image may show as broken
  if you didn't provide `figures/sample.png` — that's fine.
- The TOC appears on the page with `Introduction`, `A Slightly Longer
  ...
  each linking to the correct anchor.
