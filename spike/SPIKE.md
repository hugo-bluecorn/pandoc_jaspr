...
6. **Wire `pandoc_jaspr` into the scaffold.** The docs template
   produces a known, opinionated layout (verified against jaspr_cli
   0.23.1 / jaspr_content 0.5.2):

   ```
   spike/example_app/
     pubspec.yaml                       # jaspr, jaspr_content, jaspr_router
     analysis_options.yaml
     lib/
       main.server.dart                 # ContentApp config — EDIT THIS
       main.client.dart                 # ClientApp entrypoint — leave alone
       main.server.options.dart         # generated by jaspr_builder
       main.client.options.dart         # generated by jaspr_builder
       components/clicker.dart          # demo @client component
     content/                           # route loader scans this
       index.md                         # → /
       about.md                         # → /about
       _data/
         site.yaml                      # titleBase, favicon, social
         links.yaml                     # link aliases used in mustache
     web/
       favicon.ico
       images/logo.svg
   ```

   a. **Add the path dependency** in `spike/example_app/pubspec.yaml`:
      ...
      Then `dart pub get` from inside `spike/example_app/`.

   b. **Edit `lib/main.server.dart`.** Find the `ContentApp` block (look
      for `runApp(ContentApp(`). The template registers exactly one
      parser:
      ...
      Add `PandocParser()` alongside it. Because `PandocParser.pattern`
      matches `*.pandoc.json` and `MarkdownParser.pattern` matches
      `*.md`, the two coexist without conflict — `jaspr_content`'s
      `parsePage` extension picks the first parser whose pattern
      matches the page path:
      ...
      Don't touch `extensions:` — the template already registers
      `HeadingAnchorsExtension()` and `TableOfContentsExtension()`,
      which is exactly what the spike needs.

   c. **Drop the article into `content/`.** Either copy or symlink
      `spike/article.pandoc.json` to
      `spike/example_app/content/article.pandoc.json`. The route loader
      will expose it at `/article`. Symlink keeps a single source of
      truth:

      ```bash
      ln -s ../../article.pandoc.json spike/example_app/content/article.pandoc.json
      ```

      Also ensure `spike/figures/sample.png` is reachable from the
      content tree (symlink `spike/example_app/web/figures` to
      `spike/figures` or similar — the docs template's image handling
      uses `web/` for static assets).

   d. **Add a sidebar entry** in `lib/main.server.dart`. The template
      configures the sidebar inline:
      ...
      sidebar: Sidebar(
        groups: [
          ...
          SidebarGroup(
            title: 'Content',
            links: [SidebarLink(text: "About", href: '/about')],
          ),
        ],
      ),
      ```

      Add a `SidebarLink(text: "Article", href: '/article')` so the
      article is reachable from the navigation. The
      `TableOfContentsExtension` produces a *per-page* TOC (sections
      within the article) — that appears automatically when the page
      renders; you do not wire it into the sidebar.
